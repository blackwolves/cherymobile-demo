...
class DropdownMenu extends Component {
    ...
    renderChcek(index, title) {
        ...
        if (this.state.selectIndex[activityIndex] === index) {
            const checkImage = this.props.checkImage ? this.props.checkImage : require('../../../../img/menu_check.png');
            return (
            ...
            return (
                <View style={ { position: 'absolute', left: 0, right: 0, top: 40, bottom: 0 } }>
                  ...
                  <ScrollView style={ [{ position: 'absolute', top: 0, left: 0, right: 0, backgroundColor: 'white' }, heightStyle] }>
                    { currentTitles.map((title, index) => <TouchableOpacity
                                                                            key={ index }
                                                                            activeOpacity={ 1 }
    ...
    renderDropDownArrow(index) {
        ...
        return (
            <Animated.Image
                            source={ icon }
                            style={ { marginLeft: 8, transform: [{ rotateZ: this.state.rotationAnims[index].interpolate({ inputRange: [0, 1], outputRange: ['0deg', '360deg'] }) }] } } />
            );
    }
        ...
        return (
            <View style={ { flexDirection: 'column', flex: 1 } }>
              <View style={ { flexDirection: 'row', backgroundColor: this.props.bgColor ? this.props.bgColor : this.defaultConfig.bgColor } }>
                { this.props.data.map((rows, index) => <TouchableOpacity
                                                                         activeOpacity={ 1 }
                                                                         onPress={ this.openOrClosePanel.bind(this, index) }
                                                                         key={ index }
                                                                         style={ { flex: 1, height: 40, alignItems: "center", justifyContent: "center" } }>
                                                         <View style={ { flexDirection: 'row', alignItems: "center", justifyContent: "center" } }>
                                                           <Text style={ { color: this.props.tintColor ? this.props.tintColor : this.defaultConfig.tintColor, fontSize: 13 } }>
